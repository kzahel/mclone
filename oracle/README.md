...
it can open a real LWJGL window. `--screenshot` implies launch, creates a fresh
seeded singleplayer world, captures the vanilla main framebuffer to the named
PNG, and exits. The harness uses the deobfuscated `client-deobf.jar` plus
Mojang's 1.17.1 libraries/assets and a dummy local session, so Prism Launcher is
not part of the normal oracle command path. On Apple Silicon macOS,
`--macos-arm64-lwjgl prism` can use Prism's local arm64 LWJGL jars/natives as a
narrow compatibility layer while keeping the Minecraft client jar and harness
