...
The current vegetation set is already enough to make scenes legible, and the bee, sunflower, swamp-oak vine, non-natural deep-warm-ocean table, desert-well, monster-room, and fossil follow-through removed the clearest remaining explicit table omissions. The next leverage point is the remaining visible exactness work that is still narrow enough to validate directly:
