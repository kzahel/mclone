...
known frontier collar/skirt refinement. A post-review Explorer correction
defaults exact composition to the orbit focus while preserving the prior
camera-relative placement as an explicit diagnostic option. Child Tactical
[`266`](266-terrain-lab-runtime-composition-adoption.md) completed PH-3 and
hosted Human Review 1 accepted its reusable Terrain Lab consumer on
2026-07-27. Child Tactical
[`269`](269-shared-terrain-engine-scene-adoption.md) completed PH-4 shared
terrain-view engine extraction and full-game scene adoption. Corrective
Tactical [`271`](271-procedural-horizon-quality-baseline.md) passed hosted
Human Review on 2026-07-27. Native, browser mono, and preliminary synthetic
...
Tactical
[`277`](277-quest-procedural-horizon-performance.md) completed the first
...
profiling, and acceptance host; it is not the final gameplay owner. Explorer,
the game scene, and a future Universe overview must nevertheless be peer hosts
of one shared terrain representation/composition engine rather than separate
implementations that happen to exchange the same mask DTO.

## One-Page Feature State

```text
detached canonical source   live authoritative source   observer source
  mclone-worldgen recipe      client replica/edits       future bounded facts
               \                     |                     /
                `--------------------+--------------------'
                                     v
shared terrain representation engine (`mclone-terrain-view`)
  semantic procedural products + clipmap/residency/admission
  exact-painted coverage + target/frontier/collar
  vegetation/representation ownership + prepared terrain frame
                                     |
                 .-------------------+-------------------.
                 v                   v                   v
          World Explorer         mclone-scene       future Universe
             orbit host        live/XR host          overview host
                                     |
                                     v
render lifecycle and pipelines (`mclone-render-session` + `mclone-render`)
...
coordinator while retaining live product lifecycle. Hosted review accepted
the result with the explicitly recorded live-tree exception.
...
- Physical Quest per-eye pixels accepted in headset.
- Full-quality Quest RD5 stationary composition at `72.01 FPS` with zero
  over-period frames after shared tile culling.
- Persistent composed-horizon 8x flights over 6.2 and 10.3 kiloblocks with no
  exact-center miss, bounded queues, and stable process memory.

## What Is Not Yet Proven

- The detached exact renderer has complete natural-tree XOR, but the live
  game's ordinary chunk mesh cannot yet suppress exact tree geometry while
  the same stable record remains proxy-owned. Localized tree overlap is an
  accepted PH-4 limitation.
...
- Synthetic stereo, Quest per-eye/full-frame multiview, and Linux
  Vulkan/WiVRn desktop OpenXR are proven. Physical Metal, foveation, and
  lifecycle-recovery acceptance remain open.
- Quest stationary cost is accepted, but moving presentation retains a
  measured `2.2–5.6%` over-period tail. Phone and flat-Android budgets remain
  uncharacterized.
...
envelope and made exact natural-tree geometry and its stable record-derived
...
The hosted forest and low-angle anchors passed Human Review 1B. Tactical 266
then extracted the runtime renderer, composition session, and browser
executors into shared ownership and made Terrain Lab a second consumer.
Hosted Human Review 1 accepted that PH-3 result on 2026-07-27. PH-4 now owns
the shared-engine refactor and `mclone-scene` integration. The completed proof
boundary prevents it from combining scene lifecycle, GPU masking, and
first-pixel discovery in one cut, but `TerrainRuntimeExactRenderer` must not
become a permanent second terrain system. Canonical generation remains a
detached source adapter; live drawable sections become an authoritative
source adapter; composition, residency, ownership arbitration, and prepared
terrain-frame production converge in `mclone-terrain-view`.
...
Result: the initial hosted game review rejected coarse browser geometry,
lighting seams, cracks, and short horizon reach. Tactical 271 restored the
shared ten-level/stride-one quality baseline and passed hosted phone review on
2026-07-27. Review accepted localized overlap between a live exact tree and
its stable-record proxy as a provisional limitation; PH-5 or a later focused
child may revisit generated-feature partitioning, edit invalidation/storage,
or a rough inset proxy.
