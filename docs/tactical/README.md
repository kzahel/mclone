...
With the runtime arc accepted and tacticals 42 through 44 landed, the next highest-value worldgen/content slice should move back from generic underground helpers to remaining overworld table breadth and exactness: finish the still-thin biome-table cases, then only widen underground validation/oracle coverage if the current browser/unit surface proves insufficient.
