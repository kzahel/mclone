...
| [`ClientRuntime2-presentation-thread-boundary.md`](ClientRuntime2-presentation-thread-boundary.md) | presentation-state stream, raw input handoff, render-world/mesh handles, no raw chunk/collision ownership on UI thread | browser smoke + integration | **next** - make UI/render presentation-only without losing current debug controls |
