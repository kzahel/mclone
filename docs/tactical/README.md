...
| [`46-full-decorated-spawn-chunk-parity.md`](46-full-decorated-spawn-chunk-parity.md) | reusable full-decorated chunk diffing, seed `12345` chunk `(0, 0)` mismatch burn-down, and exact full-block server-fixture parity | integration + runtime | **next** — run mismatch burn-down on top of explicit status orchestration |
...
With the runtime arc accepted and tacticals 42 through 47 landed, the next highest-value worldgen/content slice is no longer broad biome-table breadth. Tactical [`46`](46-full-decorated-spawn-chunk-parity.md) can use the existing official-server fixture for seed `12345`, chunk `(0, 0)` as a bounded end-to-end target: promote the decorated runtime path from the current `64,768 / 65,536` full-block match to exact parity, then use the same harness to choose the next fixture set deliberately.
