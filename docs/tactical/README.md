...
With the runtime arc accepted and tacticals 42 through 48 landed, the next highest-value worldgen/content slice is no longer broad biome-table breadth. Tactical [`49`](49-vanilla-status-futures-and-partial-chunks.md) should replace the generated host's flattened authority terrain window with vanilla-shaped status futures and partial `ProtoChunk`-like records. After that, tactical [`46`](46-full-decorated-spawn-chunk-parity.md) can use the existing official-server fixture for seed `12345`, chunk `(0, 0)` as a bounded end-to-end target: promote the decorated runtime path from the current `64,790 / 65,536` full-block match to exact parity, then use the same harness to choose the next fixture set deliberately.
