...
| [`RendererHost6-generated-world-transition-scenarios.md`](RendererHost6-generated-world-transition-scenarios.md) | multi-step generated-world scenario contract for camera/chunk-interest transitions and injected input sequences | Deno WebGPU visual + browser probe | **next** — prove headless and browser harnesses can drive the same transition-shaped engine API, not only one static frame |
