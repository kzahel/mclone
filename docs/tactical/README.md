...
| [`R10-dedicated-server-ergonomics.md`](R10-dedicated-server-ergonomics.md) | `host:dedicated` alias, config-file startup tests, local-vs-dedicated browser query/UI, dedicated WebSocket smoke | unit + integration + browser smoke | **proposed** - clarify dedicated server mode without changing the authority or transport model |
