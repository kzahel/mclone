...
| [`46-full-decorated-spawn-chunk-parity.md`](46-full-decorated-spawn-chunk-parity.md) | reusable full-decorated chunk diffing, seed `12345` chunk `(0, 0)` mismatch burn-down, and exact full-block server-fixture parity | integration + runtime | **blocked** — resume after tactical 48 verifies vanilla cross-chunk `FEATURES` scheduling order |
...
| [`48-vanilla-scheduler-trace-oracle.md`](48-vanilla-scheduler-trace-oracle.md) | executable vanilla 1.17.1 scheduler trace for status task order and neighboring `FEATURES` commit order | oracle + runtime diagnostics | **next** — establish the vanilla commit-order oracle before continuing decorated block burn-down |
...
With the runtime arc accepted and tacticals 42 through 47 landed, the next highest-value worldgen/content slice is no longer broad biome-table breadth. Tactical [`48`](48-vanilla-scheduler-trace-oracle.md) should run before tactical [`46`](46-full-decorated-spawn-chunk-parity.md): it verifies the vanilla cross-chunk `FEATURES` commit order that may control the remaining tree/log/leaf differences. After that trace exists, tactical 46 can use the existing official-server fixture for seed `12345`, chunk `(0, 0)` as a bounded end-to-end target: promote the decorated runtime path from the current `64,790 / 65,536` full-block match to exact parity, then use the same harness to choose the next fixture set deliberately.
