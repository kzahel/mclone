...
With the broad layered-overworld biome key set now covered, the next highest-value parity path is a concrete decorated-stage confidence target rather than more table breadth. Tactical [`48`](48-vanilla-scheduler-trace-oracle.md) should first verify the vanilla cross-chunk `FEATURES` commit order. Then tactical [`46`](46-full-decorated-spawn-chunk-parity.md) should drive seed `12345`, chunk `(0, 0)` from the current `64,790 / 65,536` full-block runtime match to exact parity against the committed official-server fixture.
