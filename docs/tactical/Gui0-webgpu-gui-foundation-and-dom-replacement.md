...
- `debug.html` no longer owns a visible form. It can become a query-param entry into the same canvas shell or be deleted after callers migrate.
...
- Remove obsolete CSS and HTML nodes from `index.html` / `debug.html`.
