...
Do not start structures until worker-local dependency reuse is either promoted to scheduler-owned protochunk holders or explicitly accepted as sufficient for `018`, and the full chunk mismatch report is driving concrete parity work. The immediate focus is reducing the remaining top mismatch buckets for seed `12345`, chunk `0,0`, especially vegetation offsets, low-y gravel/deepslate drift, liquid-visible underground decoration, and remaining top-layer blocks, while keeping feature generation efficient enough to iterate.
