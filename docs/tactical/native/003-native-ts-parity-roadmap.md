...
After the [`025-render-section-visibility-graph.md`](025-render-section-visibility-graph.md) first pass, movement and timedemo smokes both exercise graph culling. Timedemo now loads an effective static scene radius of `max(chunk_radius, path_radius_chunks)` so the orbit stays inside retained sections. Native also mirrors Java's fallback path for cameras outside retained sections by seeding traversal from visible top/bottom retained sections. Java oracle comparison for uploaded vertex counts should wait until retained-section neighborhood policy is stable, because Java's traversal can make draw pressure differ even when chunk model compilation is correct.
