...
- no explicit protochunk holder type, fixed release-mode movement perf budgets, randomized/flowing-lava interaction `FlowingFluid` parity, unloaded-boundary/persisted liquid tick parity, or dirty-only section upload diffs
...
- no Java-style occlusion graph traversal, dirty-only section upload diffs, true liquid renderer, or AO/lighted textured path
...
| [`019-scheduler-owned-dependency-holders.md`](019-scheduler-owned-dependency-holders.md) | server runtime/perf | **done** - propagated ticket levels, lower-status holder halos, client visibility/ticking/entity-ticking splits, block-ticking chunks as live generated server chunks, scheduler-owned dependency buffers, delayed pending unloads, tick/simulation reports, worker seeding, and generation reuse counters | movement benchmark plus holder/dependency reuse/unload/ticking-lane tests |
| [`020-native-movement-perf-and-render-culling.md`](020-native-movement-perf-and-render-culling.md) | renderer/runtime/perf | **done** - circular native movement perf smoke, frustum section culling, drawn-vs-loaded section counters, and fluid-tick source notes | native circular movement smoke plus culling stats |
| [`021-scheduled-fluid-ticks.md`](021-scheduled-fluid-ticks.md) | simulation/runtime | **active** - Java-shaped fluid tick queue, generated liquid tick carry-through, level-bearing water/lava ids, Java-shaped water/lava spread helpers, exact water/lava/cross-chunk fixture matrix, chunk dirty updates, runtime smoke through simulation report | generated watery chunk smoke; exact committed liquid fixture tests |
| [`022-platform-target-contract-and-render-boundary.md`](022-platform-target-contract-and-render-boundary.md) | platform/render | explicit renderer view/target inputs, platform target contract, and desktop/headless callers kept visually stable before Android/OpenXR scaffolding | render tests, WASM check, and native headless chunk capture |
| `023-lighting-pipeline.md` | lighting | `DataLayer`, light sections, solver/service boundary, initial light facts, renderer consumption | light fixture tests and non-fullbright terrain capture |
| `024-live-light-and-liquid-updates.md` | simulation/runtime | light deltas, block/liquid dirty updates, pending liquid ticks, water flow baseline | update/delta tests and runtime smoke |
| `025-player-movement-and-collision.md` | gameplay/runtime | shared body/collision view, sequenced input commands, native prediction/reconciliation baseline | local and remote movement integration tests |
| `026-structures-foundation.md` | worldgen parity | status-aware structure starts/references, first true structure, template/block-entity scaffolding | server-backed fixture diff for first structure slice |
| `027-entities-and-passive-mobs.md` | gameplay/render | entity sections, snapshots/deltas, passive spawn baseline, first render path | generated entity fixture and visible entity capture |
| `028-native-xr-and-parity-consolidation.md` | platform/perf | OpenXR presentation smoke shape, Android XR / Quest target review, perf counters, fly-through benchmark, parity gap report | native smoke plus tracked perf/parity report |

## Immediate Focus

After the active [`021-scheduled-fluid-ticks.md`](021-scheduled-fluid-ticks.md) slice is capped or completed, the next small defensive platform slice should be [`022-platform-target-contract-and-render-boundary.md`](022-platform-target-contract-and-render-boundary.md). It should make renderer view/projection and target facts explicit before lighting, Android, or OpenXR work grows more surface area around the current desktop window path.
