...
| [`019-scheduler-owned-dependency-holders.md`](019-scheduler-owned-dependency-holders.md) | server runtime/perf | **active** - propagated ticket levels, lower-status holder halos, client visibility/ticking/entity-ticking splits, scheduler-owned dependency buffers, delayed pending unloads, tick reports, worker seeding, and generation reuse counters | movement benchmark plus holder/dependency reuse/unload/ticking-lane tests |
...
Do not start structures until the propagated residency/dependency-holder path has an explicit protochunk/dependency-holder shape and we understand the cost of cloning dependency buffers through the worker. The immediate focus is adding real tick loops on top of the now-explicit ticking/entity-ticking reports, reducing dependency-transfer overhead if it becomes material, and keeping clean `FEATURES` parity stable while deferring runtime liquid simulation tails.
