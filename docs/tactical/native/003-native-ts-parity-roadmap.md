...
- no outside-retained-section occlusion traversal seeding, true liquid renderer, or AO/lighted textured path
...
| [`025-render-section-visibility-graph.md`](025-render-section-visibility-graph.md) | renderer/perf | **first pass landed** - Java `VisGraph` / `VisibilitySet` parity, synthetic oracle fixture, conservative section occlusion traversal, client toggle, movement/timedemo culling counters, graph-build timing; outside-retained-section seeding pending | oracle fixture plus movement/timedemo smoke deltas |
| `026-lighting-pipeline.md` | lighting | `DataLayer`, light sections, solver/service boundary, initial light facts, renderer consumption | light fixture tests and non-fullbright terrain capture |
...
After the [`025-render-section-visibility-graph.md`](025-render-section-visibility-graph.md) first pass, movement and timedemo smokes both exercise graph culling. Timedemo now loads an effective static scene radius of `max(chunk_radius, path_radius_chunks)` so the orbit stays inside retained sections. The remaining Java traversal gap is the fallback path for cameras outside retained sections: Java seeds from visible in-range chunks, while native falls back to frustum-only. Java oracle comparison for uploaded vertex counts should wait until traversal and retained-section policy are stable, because Java's traversal can make draw pressure differ even when chunk model compilation is correct.

Do not start structures until the movement perf/culling harness can show whether runtime work is causing frame or remesh spikes across a larger view-distance variant, and until scheduled fluid ticks have exercised the no-op simulation lanes with a real workload. Keep clean `FEATURES` parity stable while separating initial liquid placement from live liquid simulation.
