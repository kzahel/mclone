...
- renderer face-pressure diagnostics, dense-section model-face culling tests, neighbor-aware chunk-boundary face culling, CPU-side render-section dirty cache, incremental GPU section updates, and rebuilt/uploaded vertex/face/index counters
...
- no Java-style occlusion graph traversal, true liquid renderer, or AO/lighted textured path
...
| [`023-render-mesh-culling-parity-and-perf.md`](023-render-mesh-culling-parity-and-perf.md) | renderer/perf | **done** - dense-section model-face culling tests, face-pressure diagnostics, and explicit greedy-meshing deferral | mesh/render tests plus native movement smoke |
...
| `025-lighting-pipeline.md` | lighting | `DataLayer`, light sections, solver/service boundary, initial light facts, renderer consumption | light fixture tests and non-fullbright terrain capture |
| `026-live-light-and-liquid-updates.md` | simulation/runtime | light deltas, block/liquid dirty updates, pending liquid ticks, water flow baseline | update/delta tests and runtime smoke |
| `027-player-movement-and-collision.md` | gameplay/runtime | shared body/collision view, sequenced input commands, native prediction/reconciliation baseline | local and remote movement integration tests |
| `028-structures-foundation.md` | worldgen parity | status-aware structure starts/references, first true structure, template/block-entity scaffolding | server-backed fixture diff for first structure slice |
| `029-entities-and-passive-mobs.md` | gameplay/render | entity sections, snapshots/deltas, passive spawn baseline, first render path | generated entity fixture and visible entity capture |
| `030-native-xr-and-parity-consolidation.md` | platform/perf | OpenXR presentation smoke shape, Android XR / Quest target review, perf counters, fly-through benchmark, parity gap report | native smoke plus tracked perf/parity report |
