...
Next focus is [`026-lighting-pipeline.md`](026-lighting-pipeline.md). Start with Java-shaped light storage, snapshot/protocol payloads, provisional server-owned initial light facts, and renderer handoff. Do not attempt the full `DynamicGraphMinFixedPoint` solver, AO, live light deltas, and exact `LightTexture` behavior in the same slice.
