...
- `GeneratedChunkTicketSet` now supports `sourceRadius`, so one coalesced ticket can represent a square region of vanilla source tickets and then propagate levels outward by Chebyshev distance.
- `player_view` tickets now use level `31` with the entity-ticking source radius, rather than choosing a lower center level only to make the edge land at `BORDER`.
- `GeneratedWorldHost` installs transient `light` tickets at level `33` while initial light is pending, then clears them when the light batch finishes or is cancelled. This mirrors vanilla `TicketType.LIGHT` participation in holder status without persisting light tickets after the status future completes.
...
- `pnpm vitest run test/runtime/generated-world-host-lighting-delta.test.ts`
- `pnpm vitest run test/world/level/generated-chunk-tickets.test.ts`
