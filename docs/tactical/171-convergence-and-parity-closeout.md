...
| Far LOD settle contract, correctness, and detail modes | Tactical 172 — Far LOD Settle Contract And Detail Modes | **Active; Slice 1 plus the user-testable Slice 3 chooser are landed:** Graphics exposes Auto/fixed-4/fixed-8/fixed-16 and covered-build culling. Interactive testing caught forbidden real/LOD co-rendering; final visible sets are now protected by a release-active disjointness assertion. D1/D2 remain open. | Evaluate the modes interactively, then resume Slice 2 from concrete visual observations. |
