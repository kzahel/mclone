...
| Far LOD settle contract, correctness, and detail modes | Tactical 172 — Far LOD Settle Contract And Detail Modes | **Active; Slice 1 plus the user-testable Slice 3 chooser are landed:** exact-set diagnostics and the full probe lane remain available; Graphics now exposes Auto/fixed-4/fixed-8/fixed-16 plus the normal-terrain-culling A/B. The D1/D2 coverage defects remain open. | Evaluate the modes interactively, then resume Slice 2 from concrete visual observations. |
