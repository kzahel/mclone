...
Status: active 2026-07-16. Slice 0 is complete; compiled geometry/artifact
...
- If the compiled artifact becomes the production runtime input, semantic JSON
  may become an inspection/test product instead of a second shipped runtime
  representation. Neither generated form becomes an authoring source.

### Compiler authority

- Add a deterministic offline compiler module under `tools/asset-lab`.
- Establish and preview the logical in-memory `CompiledFigure` before freezing
  its durable serialized container.
...
### Candidate artifact container

Once the logical `CompiledFigure` and compiled Asset Lab preview pass their
gates, emit one self-contained, little-endian artifact per figure so a layered
asset source cannot resolve metadata and payload from different packs. The
exact extension, Rust type names, and whether this candidate is promoted may
change based on the proof. The current candidate has:
...
### Slice 2: logical compiler and semantic validator fixture

- Define typed logical `CompiledFigure` geometry, rig, material, atlas, clip,
  bounds, count, and provenance data without binding it to a file container.
...
- Add repeated-build logical equality, golden count/bounds, Three.js box-oracle,
  malformed semantic input, and budget tests. A canonical diagnostic dump is
  acceptable for test diffs but is not the promoted runtime format.

Gate: two clean logical compiles agree exactly; the player result has 12 parts,
...
### Slice 3: Asset Lab compiled-output preview and container decision

- Build `BufferGeometry` from the logical compiled buffers, atlas, parts, and
  rest palette without calling primitive constructors.
...
- After the logical preview passes, decide whether to promote the candidate
  header/binary container. If selected, serialize it, reload it into the same
  preview, and add a CLI/package wrapper, byte-for-byte double-build tests,
  header/source-hash invalidation, malformed-container tests, and budget
  failures.
- Generate the first-party player artifact into its pack path and update the
  asset pack/lock through the normal scripts only after the serialization
  decision. Derived output is committed only with its reproducibility gate and
  source provenance.
...
exact or within the recorded floating-point tolerance. If a container is
selected, two clean serializations produce identical bytes and the reloaded
preview remains identical to the logical preview.

### Slice 4: Rust loading and first shared mono pixels
...
### Slice 5: stereo, multiview, and browser portability
...
### Slice 6: close the proof and choose the next tactical
...
Do not claim pixel completion from unit tests alone. Capture at Slice 3's first
compiled Asset Lab pixels, Slice 4's first native pixels, and Slice 5's first
