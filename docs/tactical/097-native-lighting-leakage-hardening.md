...
- Current status as of commit `2a686d5`: the one-nibble gap is guarded by the active `generated_origin_chunk_block_light_scheduler_oracle_has_one_known_edge_delta` test. The ignored strict test still fails only at section `1`, byte `1784`, expected `0x10`, got `0x00`.
- Likely cause: missing/inexact decoration parity feeding the light input world, not minimum light, gamma, moodiness, or a block-light solver leak. Java's scheduler `LIGHT` oracle appears to get that `1` block-light nibble from a neighbor-edge glow-lichen source path. Native does not reproduce the same nearby emitting source in this scheduler scenario; probes showed native has no local emitter close enough to light `(1,29,15)`.
- Revisit path: generate a focused Java/native neighbor-decoration oracle for the relevant 3x3 scheduler job, especially glow-lichen placements and spread/RNG order around chunk `(0,1)` / the southern edge of chunk `(0,0)`. Do not chase this by changing light propagation until the neighbor feature source set is proven equal.
