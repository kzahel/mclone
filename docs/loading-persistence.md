...
| Pending unload race safety | `pendingUnloads` holds a holder until `chunkToSave` is saved; returning tickets resurrect that holder | Generated host now holds pruned holders with queued generated-record saves and resurrects them before storage has the record | Still lacks stale generated-record write versions, storage epochs, and ticket-shaped unload budgets | Tactical 60 |
