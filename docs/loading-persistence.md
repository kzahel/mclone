...
| Pending unload race safety | `pendingUnloads` holds a holder until `chunkToSave` is saved; returning tickets resurrect that holder | Generated host queues holder drops with a vanilla-sized 200-holder pass budget, holds pruned holders with queued generated-record saves, and resurrects them before storage has the record | Still uses a derived authority square instead of a full ticket graph; host-side pending-write read-through is not implemented | Tactical 60 |
