...
| Status future coalescing | `ChunkHolder` owns one future per `ChunkStatus` and reuses pending/completed work | Generated host now has holder slots for preload, terrain, features, and no-light full promotion around the existing view-level jobs; recursive status scheduling is not done | Same-status work can still be shaped by view-level collection instead of direct status requests | Tactical 57 follow-up |
