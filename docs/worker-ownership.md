...
The native lighting ownership baseline now lives in `native/crates/mclone-light`
and the server light-status bridge/mailbox modules. The old browser-engine
worker service is retired and should be treated only as Git-history prior art.
The baseline is not accepted until a native post-lighting perf run records
whether host responsiveness and render/upload budgets still hold under chunk
loading. The current measured blocker is throughput during initial lit-ring
warmup, so the next ownership improvement is pipelining decoration and initial
lighting rather than adding a pool or changing transport.
