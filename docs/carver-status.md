...
4. For broader overworld recognizability, classic carvers no longer need to stay above the next biome-table slice; the cold-family cleanup is now landed, so the next broad follow-through is warm-ocean coral / sea-pickle parity or the later bamboo-jungle slice outside the narrower exhaustive-parity work.
