...
| Standalone World Explorer | native/browser proof active | `native/apps/mclone-world-explorer` is an independent native and WebGPU host over shared `mclone-terrain-view`, `mclone-view-control`, and the lightweight `mclone-render-color` contract. It links no game runtime. `pnpm native:world-explorer:smoke` validates native surface/offscreen color plus direct depth; `pnpm native:world-explorer:web:smoke` and `:mobile` validate headed desktop and Pixel 7 browser profiles. Both use one Rust session, explicit `vanilla` display-space profile across physical UNORM and sRGB targets, and the shared vegetation coordinator over a native thread or isolated browser Worker. Requested/staged/committed terrain and vegetation presentations plus scalar normal halos now retain complete forest coverage and welded lighting through transitions. Android, XR, synthetic stereo, and multiview Explorer lanes remain future work. Tacticals [`247`](tactical/247-standalone-world-explorer-foundation.md), [`249`](tactical/249-cross-platform-procedural-horizon-proof.md), [`252`](tactical/252-procedural-horizon-seams-and-transition-admission.md), [`255`](tactical/255-world-explorer-color-output-parity.md), and [`256`](tactical/256-shared-horizon-vegetation-worker-topology.md) record the current proof. |
...
`StandardGamepadSnapshot` are the shared controller contract. As of 2026-07-24,
desktop flat uses Apple GameController profiles on macOS and GilRs on
Linux/Windows; browser Rust polls W3C standard-mapped Gamepad API sources.
These adapters advertise only compatible connected devices and feed the shared
...
pnpm native:world-explorer:web:smoke:mobile
