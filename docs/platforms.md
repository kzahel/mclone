...
| Standalone World Explorer | native/browser proof active | `native/apps/mclone-world-explorer` is an independent native and WebGPU host over shared `mclone-terrain-view`, `mclone-view-control`, and the lightweight `mclone-render-color` contract. It links no game runtime. `pnpm native:world-explorer:smoke` validates native surface/offscreen color plus direct depth; `pnpm native:world-explorer:web:smoke` validates the headed browser host. Both use one Rust session, explicit `vanilla` display-space profile across physical UNORM and sRGB targets, and the shared vegetation coordinator over a native thread or isolated browser Worker. Android, XR, synthetic stereo, and multiview Explorer lanes remain future work. Tacticals [`247`](tactical/247-standalone-world-explorer-foundation.md), [`249`](tactical/249-cross-platform-procedural-horizon-proof.md), [`255`](tactical/255-world-explorer-color-output-parity.md), and [`256`](tactical/256-shared-horizon-vegetation-worker-topology.md) record the current proof. |
