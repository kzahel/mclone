...
candidate while preserving V1 as selectable and default. V2 is not yet live.**
...
| `mclone-overworld-v2` | `planned-unallocated` | The profile identity, tag, continental rules, regional archetypes, topology support, performance policy, and fixtures may be designed during Tactical 328 | Human Review D requested a selectable sibling so V1's current charm and cost remain available while the continental candidate matures; no V2 world exists yet | Allocate one unused persisted tag before first write, add shared native/Web round trips and reopen tests, keep V1 default, and advance this row to `internal-mutable` when live |
