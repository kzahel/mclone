...
flat. See the publication-valve record in
...
| P7 | Streaming throughput with Quest guardrails | Native policy | [`142`](../tactical/142-throughput-policy-with-quest-rd5-guardrail.md), [`140`](../tactical/140-streaming-throughput-frame-pacing-baselines.md), [`../performance-records.md`](../performance-records.md), [`../quest-standalone-performance-records.md`](../quest-standalone-performance-records.md) | active strategy | Optimize desktop RD10/RD15 startup-streaming throughput, not RD5. Quest RD5 stays the safety guardrail; Quest RD7 is the pressure check for shared policy changes. Falsification is done (2026-07-04): the publish-budget prototype delivered `3x` RD10 full-view-ready with clean pacing, the host-only cadence lever is retired, and workers do not move the bulk mesh drain. Next: implement the elapsed-budget publication drain behind a throughput profile (142 Candidate A) with per-stage publish/queue instrumentation, then mesh-drain scan attribution (Candidate B). |
