...
RD10 prebuilt mesh footprint is `0.133s` for `360 MB`. The desktop-shaped
persisted startup-streaming lane now ties those splits back to the real frame
loop: RD10 already-generated reopen is playable in `0.125s`, reaches full
target view in `1.031s`, and reaches stable actionable render idle in
`25.142s` with `0/2400` over-budget frames. Remaining work is the Quest
persisted-world guardrail plus render-admission/mesh-tail attribution.
...
| P7 | Streaming throughput with Quest guardrails | Native policy | [`142`](../tactical/142-throughput-policy-with-quest-rd5-guardrail.md), [`140`](../tactical/140-streaming-throughput-frame-pacing-baselines.md), [`../performance-records.md`](../performance-records.md), [`../quest-standalone-performance-records.md`](../quest-standalone-performance-records.md) | active strategy | Optimize desktop RD10/RD15 startup-streaming throughput, not RD5. Quest RD5 stays the safety guardrail; Quest RD7 plus a streaming lane answers whether shared policy changes are safe, not whether desktop-proven publication is automatically a Quest win. Falsification is done (2026-07-04): the publish-budget prototype delivered `3x` RD10 full-view-ready with clean pacing, the host-only cadence lever is retired, and synthetic bulk workers did not move mesh drain. The 2026-07-05 sweep says Candidate A should be a shared elapsed-budget drain calculation that opens only to the measured need (`~4` publications/tick on the desktop lane), not a large fixed count. The raw/server ceiling split says raw feature generation is much higher than desktop-shaped throughput, while server-only lighting is a real cost. Persisted reload bounds already-generated server loading below `1s` at RD10, mesh CPU-only costs `11.037s`, GPU upload-only is `0.133s`, and desktop-shaped persisted startup-streaming reaches playable/full-view in `0.125s`/`1.031s` but stable actionable render idle in `25.142s`; `pnpm native:android-xr:perf:orbit:rd5:persisted` now packages the Quest persisted guardrail row, and the next work is to run it plus render-admission/mesh-tail attribution before changing broad policy. |
