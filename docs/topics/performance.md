...
Next decomposition is persisted and render-only lanes: in-memory persisted
server reload now measures RD10 view-ready at `0.523s` from `625` already-lit
records with no worldgen/light work; remaining splits are temp SQLite persisted
server reload, mesh CPU-only, GPU upload-only, then a Quest persisted-world
guardrail once desktop costs are understood.
