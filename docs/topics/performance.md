...
Remaining work is no longer "prove the baseline"; it is a new per-stage render
pipeline budgeting tactical covering worker count, render admission,
completed-result acceptance, GPU upload/apply, XR accept/upload policy, and
remote/local host-mode treatment.
...
| P7 | Streaming throughput with Quest guardrails | Native policy | [`142`](../tactical/142-throughput-policy-with-quest-rd5-guardrail.md), [`150`](../tactical/150-adaptive-frame-budget-controller.md), [`../performance-records.md`](../performance-records.md), [`../quest-standalone-performance-records.md`](../quest-standalone-performance-records.md) | baseline stabilized; follow-up tactical needed | Optimize desktop RD10/RD15 startup-streaming throughput, not RD5. Tactical 150 promoted adaptive publication for local-integrated desktop/native-XR/Android-XR and the conservative render compile queue-depth baseline (`workers=1`, `max-pending=4`). Clean Slice 5 rows show RD10 frozen full-view `9.14-9.18s`, RD15 full-view `19.43s`, persisted RD10 target quiescence `4.925s`, and short Quest RD5/RD7/churn guardrails green. The long Quest churn soak was not a clean pass (`41.79` submitted FPS, periodic dropped-frame deltas grew, queue ages rose), and movement-frame had one repeatable legacy over-2x spike. The next work should be a per-stage render pipeline budgeting tactical, not more broad default promotion inside 150. |
