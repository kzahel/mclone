...
worldgen/light work. Mesh CPU-only from persisted snapshots is now measured at
`11.037s` for `8464` RD10 target sections, while GPU upload-only for the same
RD10 prebuilt mesh footprint is `0.133s` for `360 MB`. Remaining work is a
persisted-world startup-streaming lane and Quest persisted-world guardrail.
