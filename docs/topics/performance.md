...
| P1 | Liquid renderer light sampling | Yes | [`054`](../tactical/054-native-block-render-facts-parity.md), [`lighting topic`](lighting.md) | candidate Mclone visual-lighting work | Stored sky/block values reach the renderer, the shader uses the retained Java-shaped lightmap curve, and mesh-side AO includes non-cubic shape weighting. Liquid light sampling remains absent and should be prioritized by product-visible impact, not Java parity. |
| P2 | Sky neighbor skip-through propagation | Yes | [`049`](../tactical/049-native-sky-source-section-ownership.md), [`lighting topic`](lighting.md) | optional solver comparison | Source-section ownership now lives in `SkyLightSectionStorage`, but native differs from Java `SkyLightEngine.checkNeighborsAfterUpdate(...)` across vertical light-storage gaps. Adopt comparable behavior only if Mclone live deltas or visuals need it. |
| P3 | Live light deltas and light-section dirtying | Yes | [`lighting topic`](lighting.md), [`026`](../tactical/026-lighting-pipeline.md), [`031`](../tactical/031-native-section-block-delta-updates.md) | pending larger subsystem | Section block deltas currently leave light payloads unchanged. Correct live lighting needs engine-owned propagation/deltas and render dirtying by changed light sections. |
