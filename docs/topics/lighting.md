...
now uses native graph-driven light engines over loaded chunk block facts. Light
is a real chunk fact that can cross the native server/client/render boundary,
but full Java-shaped light status scheduling is still pending.
...
- `ChunkStatus::Light` exists, but the native server does not run a real light
  status.
- There is no full Java-shaped `LayerLightEngine`, `LevelLightEngine`, or
  threaded light scheduling wrapper yet.
...
On 2026-06-18, after the graph-backed sky-light bridge landed, a native
...
| graph-backed sky light bridge | `native/crates/mclone-server/src/sky_light_bridge.rs` |
