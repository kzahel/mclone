...
The first storage/render handoff has landed, and runtime block light now uses
the native graph-driven block-light foundation over loaded chunk block facts.
Light is a real chunk fact that can cross the native server/client/render
boundary, but sky light still uses the provisional producer and full
Java-shaped light status scheduling is still pending.
...
- Sky light is still provisionally seeded/flood-filled across the available 3x3
  chunk neighborhood.
...
- `ChunkStatus::Light` exists, but the native server does not run a real light
  status.
- There is no full Java-shaped `LayerLightEngine`, `SkyLightSectionStorage`,
  `SkyLightEngine`, `LevelLightEngine`, or threaded light scheduling wrapper
  yet.
...
On 2026-06-18, after the graph-backed block-light bridge landed, a native
...
| sky seed producer | `native/crates/mclone-server/src/lighting_seed.rs` |
| graph-backed block light bridge | `native/crates/mclone-server/src/block_light_bridge.rs` |
