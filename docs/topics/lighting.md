...
- The native `ChunkStatus::Light` pass is synchronous in scheduler polling; it
  is not yet a full threaded/native-worker equivalent to
  `ThreadedLevelLightEngine`.
- There is no full Java-shaped `LayerLightEngine` abstraction yet, and the new
  `LevelLightEngine` does not yet expose `queueSectionData` / `retainData`.
...
On 2026-06-18, after native `ChunkStatus::Light` scheduling landed, a native
...
The previous `LevelLightEngine` bridge capture was:
...
| pending light status inputs | `native/crates/mclone-server/src/light_status.rs` |
...
Remaining parity gaps: light propagation is still synchronous inside scheduler
polling, Java `queueSectionData` / `retainData` are still pending, and loaded
light data hydration does not yet use solver-owned storage.
...
- Add `queueSectionData` / `retainData` hooks to the native layer engines and
  coordinator.
- Preserve persisted `ChunkStatus::Light` snapshots as valid light data instead
  of treating saved payloads as opaque cache bytes.
- Keep loaded light ownership in `mclone_light`, not in mesh/render code.
- Keep the scheduler-owned `ChunkStatus::Light` status boundary from P5.
