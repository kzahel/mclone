...
> Refreshed again on 2026-08-16 after Tactical 310 implemented one resolved
> local-session launch plan for native and Web, preserved full-width browser
> seeds, and passed non-default mutation plus untouched default-menu product
> gates. The parent platform-boundary topic remains open only for its required
> separately staffed fixpoint audit; the local-session consumer row is no
> longer partial.
...
- **web** uses the same shared scene-policy host and resolved local-session
  launch plan as the other clients. Tactical 310 removed the last-mile
  semantic projector, preserved exact signed 64-bit catalog seeds, and added
  default-Mclone-Wild and non-default mutation gates. Other product gaps
  include connect-screen text/smoke, full persistence
...
Android, desktop XR, Android XR, and web share `mclone-scene` and one resolved
local-session launch plan; browser mechanisms stay typed services, while
...
9. **Independently audit shared local-session semantic lowering.** Tactical
   310 now resolves one nested launch plan before native/Web resource binding,
   classifies render capacity with an applied/normalized receipt, and passes
   non-default mutation plus real default-menu gates. The implementation
   tactical is complete; the reopened parent requires a separately staffed
   fixpoint audit before its status may close again.
