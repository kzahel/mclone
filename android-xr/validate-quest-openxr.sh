...
if [[ -n "$PERF_SECONDS" ]]; then
    ...
    grep -E "MCLONE_ANDROID_XR_PERF_(SUMMARY|HEADROOM|STAGES|TERRAIN|TERRAIN_RUNTIME|TERRAIN_PREP|OVERLAP|MULTIVIEW|UPLOAD_MAX|RECORD_CACHE|RUNTIME_MAX|QUEUE_MAX|COMPILE_MAX|UPLOAD_LAST|DRAW)([[:space:]]|$)|MCLONE_ANDROID_XR_PERF_METRICS[[:space:]]" "$LOG_PATH" \
        | tail -n 16 > "$PERF_SUMMARY_PATH"
