...
if [[ -n "$PERF_SECONDS" ]]; then
    ...
    grep -E "MCLONE_ANDROID_XR_PERF_(SUMMARY|HEADROOM|STAGES|TERRAIN|TERRAIN_RUNTIME|UPLOAD_APPLY_MAX|TERRAIN_PREP|OVERLAP|MULTIVIEW|UPLOAD_MAX|UPLOAD_PHASE_MAX|RECORD_CACHE|RUNTIME_MAX|UPDATE_APPLY_MAX|QUEUE_MAX|COMPILE_MAX|UPLOAD_LAST|DRAW|WORST_FRAME_UPLOAD_APPLY)([[:space:]]|$)|MCLONE_ANDROID_XR_PERF_METRICS[[:space:]]" "$LOG_PATH" \
        | tail -n 24 > "$PERF_SUMMARY_PATH"
