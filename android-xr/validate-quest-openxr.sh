...
if [[ -n "$PERF_SECONDS" ]]; then
    ...
    grep -E "MCLONE_ANDROID_XR_PERF_(SUMMARY|STAGES|TERRAIN|TERRAIN_PREP|MULTIVIEW|UPLOAD_MAX|RUNTIME_MAX|QUEUE_MAX|COMPILE_MAX|UPLOAD_LAST|DRAW)" "$LOG_PATH" \
        | tail -n 11 > "$PERF_SUMMARY_PATH"
