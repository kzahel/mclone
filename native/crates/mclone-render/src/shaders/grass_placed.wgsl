...
struct VertexOutput {
    ...
    @location(2) composition_position: vec3<f32>,
};
...
fn lightmap_color(block_light_level: f32, sky_light_level: f32) -> vec3<f32> {
...
fn apply_color_profile(color: vec3<f32>) -> vec3<f32> {
...
fn grass_interaction_bend(world_sample: vec2<f32>) -> vec2<f32> {
    ...
    let lifted = vec2<f32>(
        interaction_nearest_periodic_lift(
            world_sample.x,
            center.x,
...
@vertex
fn vs_main(input: GrassPatchInput, @builtin(vertex_index) vertex_index: u32) -> VertexOutput {
    var output: VertexOutput;
    let source_position = blade_vertex(input, vertex_index);
    ...
    let sky_light = f32((input.packed_light >> 20u) & 15u);
    let light = lightmap_color(block_light, sky_light);
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    let color = apply_color_profile(
        apply_fog(input.color * input.light, input.composition_position),
    );
