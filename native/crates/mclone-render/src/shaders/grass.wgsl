...
fn vs_main(input: GrassPatchInput, @builtin(vertex_index) vertex_index: u32) -> VertexOutput {
    ...
    let world_position = observer_local_position(blade_vertex(input, vertex_index));
...
fn fs_main(input: VertexOutput) -> @location(0) vec4<f32> {
    let color = apply_color_profile(apply_fog(input.color * input.light, input.world_position));
