...
pub const LOCAL_REALM_BOOTSTRAP_SAVED_DATA_KEYS: [&str; 3] = [
    REALIZED_STARTER_PLAN_SAVED_DATA_KEY,
    DEER_POPULATION_HISTORY_KEY,
    crate::wildlife_resources::WILDLIFE_RESOURCE_SAVED_DATA_KEY,
];
...
impl RealmServer {
    ...
    pub fn wildlife_forage_cells(&self) -> Vec<crate::WildlifeForageCellSnapshot> {
        ...
        self.active_dimension
            ...
            .snapshots_with_opportunity(&|position, kind| {
                opportunity_cache.opportunity(position, kind)
            })
    }
    ...
    pub(crate) fn try_closed_wildlife_ecology_tick(
        ...
        entity_updates.extend(runtime.entities.tick_wildlife_lifecycle_with_opportunity(
            ...
            &|position, kind| seasonal_resource_opportunities.opportunity(position, kind),
            &|pos| {
                entity_ticking_set
    ...
    fn bind_intro_homestead_plan_metadata(
            ...
            .save_world_metadata_blocking(metadata.clone())?
        {
            ...
            StoreWriteOutcome::SkippedCachePressure => Err(ChunkStoreError::InvalidData(
                "durable starter-plan metadata save was skipped by cache pressure".to_owned(),
            )),
    ...
    fn handle_attack_entity_for_target(
                ...
                })
        {
            ...
            let Some(result) = self
                .active_dimension
                .entities
                .damage_habitat_prop(command.target)
            else {
                return Ok(Vec::new());
            };
            self.dirty_entity_chunks.insert(before_chunk);
            for update in &result.updates {
                self.dirty_entity_chunks.insert(update.chunk_pos());
            }
            self.mark_entity_updates_dirty(&result.updates);
