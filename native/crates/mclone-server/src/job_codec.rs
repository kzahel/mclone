...
impl From<AlphaFeatureDependencyCacheReport> for GenerationCacheReport {
    fn from(report: AlphaFeatureDependencyCacheReport) -> Self {
        Self {
            requested_dependency_chunks: report.requested_dependency_chunks,
            cache_hits: report.cache_hits,
            generated_dependency_chunks: report.generated_dependency_chunks,
...
impl From<BetaFeatureDependencyCacheReport> for GenerationCacheReport {
    fn from(report: BetaFeatureDependencyCacheReport) -> Self {
        Self {
            requested_dependency_chunks: report.requested_dependency_chunks,
            cache_hits: report.cache_hits,
            generated_dependency_chunks: report.generated_dependency_chunks,
...
impl From<ContinentalCandidateFeatureDependencyCacheReport> for GenerationCacheReport {
    ...
    pub(crate) fn retained_chunk_count(&self, profile: WorldGenerationProfile) -> usize {
        match profile {
            ...
            WorldGenerationProfile::McloneOverworldV2 => {
                self.mclone_overworld_v2_cache.as_ref().map_or(
                    0,
    ...
    pub(crate) fn execute(
        ...
        match descriptor.profile {
            ...
            WorldGenerationProfile::McloneOverworldV2 => {
                if !dependencies.is_empty() {
                    return Err(format!(
                        "mclone-overworld-v2 owns internal surface dependencies but received {} seeded chunks",
                        dependencies.len()
                    ));
                }
                if self
                    ...
                    .as_ref()
                    .is_none_or(|cache| cache.generator().seed() != descriptor.seed)
                {
                    self.mclone_overworld_v2_cache = Some(
                        ContinentalCandidateFeatureDependencyCache::new(descriptor.seed),
                    );
                }
                let cache = self
                    .mclone_overworld_v2_cache
            ...
            WorldGenerationProfile::AlphaV1 { winter } => {
                let result = self.alpha_cache.generate_features_chunks_with_dependencies(
                    descriptor.seed,
                    winter,
                    targets.iter().copied(),
                    dependencies,
            ...
            WorldGenerationProfile::BetaV1 => {
                let result = self.beta_cache.generate_features_chunks_with_dependencies(
                    descriptor.seed,
                    targets.iter().copied(),
                    dependencies,
                );
...
mod tests {
    ...
    #[test]
    fn mclone_overworld_v2_frame_generates_continental_exact_chunks() {
        let target = ChunkPos::new(17, -23);
        let descriptor =
            WorldGenerationDescriptor::new(WorldGenerationProfile::McloneOverworldV2, 12_345);
        let request = full_worldgen_frame(ChunkJobId(19), descriptor, &[target]).unwrap();
        let decoded =
            decode_worldgen_response(&compute_worldgen_job_frame(&request).unwrap()).unwrap();
        let actual = decoded
            .generated_chunks
            .get(&target)
            .expect("V2 response contains its requested target");
        let expected = mclone_worldgen::levelgen::generate_continental_candidate_chunk(
            descriptor.seed,
            target.x,
            target.z,
        );
    ...
    fn diagnostics_follow_the_profile_contract() {
        ...
        let alpha = full_worldgen_frame(
            ChunkJobId(11),
            WorldGenerationDescriptor::new(WorldGenerationProfile::alpha_v1(false), 12_345),
            &target,
        )
        .and_then(|frame| compute_worldgen_job_frame(&frame))
        .and_then(|frame| decode_worldgen_response(&frame))
        .unwrap();
        let diagnostics = alpha.diagnostics.expect("alpha cache diagnostics");
        assert_eq!(diagnostics.cache_report.requested_dependency_chunks, 25);
        assert!(diagnostics.overworld_timing.is_none());

        let beta = full_worldgen_frame(
            ChunkJobId(12),
            WorldGenerationDescriptor::new(WorldGenerationProfile::BetaV1, 12_345),
            &target,
        )
        .and_then(|frame| compute_worldgen_job_frame(&frame))
        .and_then(|frame| decode_worldgen_response(&frame))
        .unwrap();
        let diagnostics = beta.diagnostics.expect("Beta cache diagnostics");
        assert_eq!(diagnostics.cache_report.requested_dependency_chunks, 25);
        assert!(diagnostics.overworld_timing.is_none());
    }
    ...
    fn mclone_overworld_frames_are_dependency_bearing_partition_and_order_independent() {
        ...
        let batch = full_worldgen_frame(ChunkJobId(1), descriptor, &targets)
            ...
            .and_then(|frame| decode_worldgen_response(&frame))
            .unwrap();
        let reversed = full_worldgen_frame(
            ChunkJobId(2),
            descriptor,
        ...
        let changed = full_worldgen_frame(ChunkJobId(9), changed_descriptor, &targets)
            .and_then(|frame| compute_worldgen_job_frame(&frame))
            .and_then(|frame| decode_worldgen_response(&frame))
            .unwrap();
        assert_ne!(batch.generated_chunks, changed.generated_chunks);
    ...
    fn alpha_frames_preserve_winter_and_batch_order() {
        ...
        let temperate_descriptor =
            WorldGenerationDescriptor::new(WorldGenerationProfile::alpha_v1(false), 12_345);
        let forward = full_worldgen_frame(ChunkJobId(20), temperate_descriptor, &targets)
            .and_then(|frame| compute_worldgen_job_frame(&frame))
            .and_then(|frame| decode_worldgen_response(&frame))
            .unwrap();
        let reversed = full_worldgen_frame(
            ChunkJobId(21),
            temperate_descriptor,
            &[targets[1], targets[0]],
        )
        .and_then(|frame| compute_worldgen_job_frame(&frame))
        .and_then(|frame| decode_worldgen_response(&frame))
        ...
        let winter_descriptor =
            WorldGenerationDescriptor::new(WorldGenerationProfile::alpha_v1(true), 12_345);
        let winter = full_worldgen_frame(ChunkJobId(22), winter_descriptor, &targets)
            .and_then(|frame| compute_worldgen_job_frame(&frame))
            .and_then(|frame| decode_worldgen_response(&frame))
            .unwrap();
        assert_eq!(winter.descriptor, winter_descriptor);
        assert_ne!(forward.generated_chunks, winter.generated_chunks);
    }

    #[test]
    fn beta_frames_are_dependency_bearing_partition_and_order_independent() {
        let descriptor = WorldGenerationDescriptor::new(WorldGenerationProfile::BetaV1, 12_345);
        let targets = [ChunkPos::new(0, 0), ChunkPos::new(-3, 5)];
        let batch = full_worldgen_frame(ChunkJobId(23), descriptor, &targets)
            .and_then(|frame| compute_worldgen_job_frame(&frame))
            .and_then(|frame| decode_worldgen_response(&frame))
            .unwrap();
        let reversed = full_worldgen_frame(ChunkJobId(24), descriptor, &[targets[1], targets[0]])
            .and_then(|frame| compute_worldgen_job_frame(&frame))
            .and_then(|frame| decode_worldgen_response(&frame))
            .unwrap();
        let partitioned = targets
            ...
            .enumerate()
            .flat_map(|(index, target)| {
                full_worldgen_frame(ChunkJobId(index as u64 + 25), descriptor, &[target])
                    .and_then(|frame| compute_worldgen_job_frame(&frame))
                    .and_then(|frame| decode_worldgen_response(&frame))
                    .unwrap()
                    .generated_chunks
        ...
        let changed_descriptor =
            WorldGenerationDescriptor::new(WorldGenerationProfile::BetaV1, 12_346);
        let changed = full_worldgen_frame(ChunkJobId(27), changed_descriptor, &targets)
            .and_then(|frame| compute_worldgen_job_frame(&frame))
            .and_then(|frame| decode_worldgen_response(&frame))
            .unwrap();
        assert_ne!(batch.generated_chunks, changed.generated_chunks);
