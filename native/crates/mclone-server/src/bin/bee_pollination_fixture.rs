...
const DEFAULT_ROOT: &str = "/tmp/mclone-bee-pollination-fixture";
const PROFILE_ID: &str = "6d636c6f-6e65-4265-a550-6f6c6c696e61";
const PROFILE_ID_BYTES: [u8; 16] = [
    0x6d, 0x63, 0x6c, 0x6f, 0x6e, 0x65, 0x42, 0x65, 0xa5, 0x50, 0x6f, 0x6c, 0x6c, 0x69, 0x6e, 0x61,
];
...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let identity = ClientIdentity::new(PlayerProfileId::new(PROFILE_ID_BYTES), "Pollinator")?;

    write_authored_world_fixture_dir(&world_dir, AuthoredWorldFixtureKind::BeeFloweringMeadow)?;
    write_profile(&profile_path)?;
    let mut store = SqliteWorldStore::open_world_dir(&world_dir)?;
    let manifest = write_playable_showcase_to_store(
        &mut store,
        PlayableShowcaseId::BeePollination,
        &identity,
    )?;
    store.close()?;

    println!(
        "MCLONE_PLAYABLE_SHOWCASE_FIXTURE id={} revision={} dir={} profile={} seed={} day_time={} freeze_time={} entry_eye={} entry_target={} entity_count={} bee_count={} bee_nest_count={} bee_hotel_count={} bee_field_guide_bits={}",
        manifest.id.label(),
        manifest.revision,
        world_dir.display(),
        ...
        manifest.bee_count,
        manifest.bee_nest_count,
        manifest.bee_hotel_count,
...
fn parse_root(args: impl IntoIterator<Item = String>) -> Result<PathBuf, Box<dyn Error>> {
    ...
    while let Some(arg) = args.next() {
        match arg.as_str() {
            ...
            "--help" | "-h" => {
                println!(
                    "bee_pollination_fixture [--root PATH]\n\nCompiles the bee-pollination playable showcase into a disposable SQLite world for native capture.\nDefault root: {DEFAULT_ROOT}"
                );
                return Ok(root);
            }
