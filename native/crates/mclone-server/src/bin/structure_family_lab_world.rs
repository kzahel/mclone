...
#[cfg(not(target_arch = "wasm32"))]
use mclone_server::write_structure_family_lab_dir;

#[cfg(not(target_arch = "wasm32"))]
const DEFAULT_WORLD_ROOT: &str = "/tmp/mclone-structure-family-lab-v1";

#[cfg(not(target_arch = "wasm32"))]
fn main() -> Result<(), Box<dyn Error>> {
    let root = parse_root(std::env::args().skip(1))?;
    let manifest = write_structure_family_lab_dir(&root)?;
    println!(
        "MCLONE_STRUCTURE_FAMILY_LAB id={} seed={} generation={} dir={} spawn={:?} placements={}",
        manifest.gallery_id,
        manifest.seed,
...
fn main() {
    panic!("structure_family_lab_world is a native-only authoring utility");
}
