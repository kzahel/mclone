...
impl WorldStoreRequest {
    ...
    fn owned_bytes_estimate(&self) -> usize {
        match self {
            ...
            Self::SaveSavedData { record, .. } => std::mem::size_of_val(record)
                .saturating_add(record.key.capacity())
                .saturating_add(record.bytes.capacity()),
...
fn threaded_request_lane(request: &WorldStoreRequest) -> ThreadedRequestLane {
    match request {
        ...
        | WorldStoreRequest::SaveSavedData { .. }
...
mod tests {
    ...
    fn binary_world_generation_profile_discriminants_are_stable() {
        ...
        assert!(
            read_world_generation_profile(&mut [9].as_slice())
                .unwrap_err()
                .to_string()
                .contains("unknown world generation profile tag 9")
        );
    }
