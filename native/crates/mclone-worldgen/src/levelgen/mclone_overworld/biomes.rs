...
pub fn mclone_overworld_biome_id(seed: i64, world_x: i32, world_z: i32) -> i32 {
    biome_id_for_sample(McloneOverworldSampler::new(seed).sample(world_x, world_z))
}

pub(super) fn biome_id_for_sample(sample: McloneOverworldTerrainSample) -> i32 {
...
mod tests {
    ...
    fn biome_language_separates_water_shore_lowland_and_wooded_upland() {
        assert_eq!(biome_id_for_sample(sample(61)), OCEAN_BIOME_ID);
        assert_eq!(biome_id_for_sample(sample(62)), BEACH_BIOME_ID);
        assert_eq!(biome_id_for_sample(sample(66)), BEACH_BIOME_ID);
        assert_eq!(biome_id_for_sample(sample(67)), PLAINS_BIOME_ID);
        assert_eq!(biome_id_for_sample(sample(74)), PLAINS_BIOME_ID);
        assert_eq!(
            biome_id_for_sample(sample(75)),
            MCLONE_OVERWORLD_FOREST_BIOME_ID
        );
    }
}
