...
fn compile_render_section_request(
    ...
    let result = build_render_sections_from_snapshots_with_biome_zoom_seed_topology_and_grass(
        &request.snapshots,
        catalog,
        &request.target_sections,
        request.biome_zoom_seed,
        request.topology,
        request.grass_patches,
    )
...
mod tests {
    ...
    fn textured_mesh_assets_load_from_real_extracted_assets_when_present() {
        ...
        assert!(assets.catalog.get(mclone_core::BlockStateId(153)).is_some());
        assert!(assets.catalog.get(mclone_core::BlockStateId(154)).is_some());
        assert!(assets.catalog.get(mclone_core::BlockStateId(155)).is_some());
        assert!(assets.catalog.get(mclone_core::BlockStateId(156)).is_some());
    }
