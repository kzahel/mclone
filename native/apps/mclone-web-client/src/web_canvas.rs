...
use mclone_app_runtime::scene_session_runtime::{
    RuntimeRenderPriority, SceneRuntimeService, SceneSessionRuntime, StartupReadinessPolicy,
};
...
// magic, u32 generation, u32 flags (bit 0 = reset/full-resync, bit 1 = biome
// seed, bit 2 = grass patches), two fixed-width encoded
...
const WEB_RENDER_COMPILE_DELTA_FLAG_GRASS_PATCHES: u32 = 4;
...
impl WebStartupConfig {
    ...
    #[wasm_bindgen(getter, js_name = showcaseDeerCount)]
    pub fn showcase_deer_count(&self) -> Option<u32> {
        self.showcase
            .and_then(|showcase| playable_showcase_manifest(showcase).ok())
            .and_then(|manifest| u32::try_from(manifest.deer_count).ok())
    }

    #[wasm_bindgen(getter, js_name = showcaseDeerBedCount)]
    pub fn showcase_deer_bed_count(&self) -> Option<u32> {
        self.showcase
            .and_then(|showcase| playable_showcase_manifest(showcase).ok())
            .and_then(|manifest| u32::try_from(manifest.deer_bed_count).ok())
    }

    #[wasm_bindgen(getter, js_name = showcaseDeerFieldGuideBits)]
    pub fn showcase_deer_field_guide_bits(&self) -> Option<u32> {
        self.showcase
            .and_then(|showcase| playable_showcase_manifest(showcase).ok())
...
impl WebRenderCompilerSession {
    ...
    pub fn new_selected(
        ...
        diagnostic_pack_bytes: js_sys::Uint8Array,
        profile_id: String,
        ...
        let asset_pack_byte_length = authored_pack_bytes.length() as usize
            ...
            + diagnostic_pack_bytes.length() as usize;
        let profile = TextureVisualProfile::parse(&profile_id)
            .ok_or_else(|| JsValue::from_str("unknown visual profile id"))?;
        let presentation = TexturePresentation::parse(&presentation_id)
            .ok_or_else(|| JsValue::from_str("unknown texture presentation id"))?;
...
impl WebRenderCompilerSession {
    ...
    fn apply_render_compile_delta(&mut self, delta: WebRenderCompileDelta) -> Result<(), String> {
        ...
        self.grass_patches = delta.grass_patches;
...
fn encode_web_render_compile_delta(
    ...
    if grass_patches {
        flags |= WEB_RENDER_COMPILE_DELTA_FLAG_GRASS_PATCHES;
    }
...
fn decode_web_render_compile_delta(bytes: &[u8]) -> Result<WebRenderCompileDelta, String> {
    ...
    let grass_patches = flags & WEB_RENDER_COMPILE_DELTA_FLAG_GRASS_PATCHES != 0;
...
fn packed_compile_report_summary_to_js(
    ...
    set_number(
        &object,
        "grassPatchCount",
        f64::from(summary.grass_patch_count),
    )?;
...
impl WebSharedCompileInFlight {
    fn request_id(&self) -> u32 {
        match self {
            Self::Sections { request_id, .. } => *request_id,
        }
    }
}
...
impl WebRenderSectionCompiler {
    ...
    fn fail_in_flight(&mut self, message: String) {
        ...
        let WebSharedCompileInFlight::Sections {
            target_sections,
            section_revisions,
            ..
        } = in_flight;
    ...
    fn poll_completed_work(&mut self) -> anyhow::Result<()> {
        ...
        match status {
            RENDER_COMPILER_SHARED_RESULT_COMPLETE => {
                ...
                self.completed_sections
                    .push_back(render_section_compile_result_from_report(
                        target_sections,
                        section_revisions,
                        section_report,
                    ));
            ...
            RENDER_COMPILER_SHARED_RESULT_FAILED => {
                ...
                self.completed_sections
                    .push_back(RenderSectionCompileResult {
                        target_sections,
                        section_revisions,
                        result: Err("render compiler worker reported a failed compile".to_owned()),
                    });
            }
...
impl WebSceneRuntimeService {
    pub fn new(
        ...
        runtime.scene_core_mut().set_monotonic_clock(clock);
...
fn load_selected_web_mesh_assets(
    ...
    let file_count = [
        selection
            ...
            .then_some(&authored_bytes),
        selection
            .is_enabled(&mclone_assets::AssetPackId::new(
                mclone_assets::MINECRAFT_REFERENCE_PACK_ID,
            ))
            .then_some(&reference_bytes),
        selection
            .is_enabled(&mclone_assets::AssetPackId::new(
                mclone_assets::PROVISIONAL_FIRST_PARTY_PACK_ID,
            ))
            .then_some(&fallback_bytes),
        selection
            .is_enabled(&mclone_assets::AssetPackId::new(
                mclone_assets::DIAGNOSTIC_MISSING_PACK_ID,
            ))
            .then_some(&diagnostic_bytes),
    ]
    .into_iter()
    .flatten()
    .map(|bytes| {
...
pub(super) fn ui_action_label(action: GameUiAction) -> &'static str {
    match action {
        ...
        GameUiAction::CycleWorldStarterContent => "cycleWorldStarterContent",
        GameUiAction::ApplyHomesteadShowcasePreset => "applyHomesteadShowcasePreset",
        ...
        GameUiAction::CycleTexturePresentation => "cycleTexturePresentation",
        ...
        GameUiAction::StageTerrainLodPreset(_) => "stageTerrainLodPreset",
        GameUiAction::ApplyTerrainLodPreset => "applyTerrainLodPreset",
        GameUiAction::CancelTerrainLodPreset => "cancelTerrainLodPreset",
        ...
        GameUiAction::SetLocalPlayLayout(_) => "setLocalPlayLayout",
        GameUiAction::ToggleLocalPlayGuest => "toggleLocalPlayGuest",
...
mod tests {
    ...
    fn web_render_compile_delta_roundtrips_upserts_and_evictions() {
        ...
        let bytes = encode_web_render_compile_delta(
            7,
            true,
            Some(1124),
            topology,
