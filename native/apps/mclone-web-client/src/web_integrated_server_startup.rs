...
const fn playable_showcase_tag(showcase: Option<mclone_server::PlayableShowcaseId>) -> u8 {
    match showcase {
        ...
        Some(mclone_server::PlayableShowcaseId::SquirrelWoodland) => 7,
    }
}
...
fn playable_showcase_from_tag(
    ...
    match tag {
        ...
        7 => Ok(Some(mclone_server::PlayableShowcaseId::SquirrelWoodland)),
        _ => Err(format!(
...
mod tests {
    ...
    #[test]
    fn startup_profile_tag_nine_is_mclone_overworld_v2() {
        assert_eq!(
            generation_profile_tag(WorldGenerationProfile::McloneOverworldV2),
            9
        );
        assert_eq!(
