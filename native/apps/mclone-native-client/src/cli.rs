...
const MAX_WINDOW_FRAME_REPORT_SECONDS: f64 = 3600.0;
...
pub(crate) struct HeadlessScreenshotOptions {
    ...
    pub(crate) worldgen_lens: Option<WorldgenLensLayer>,
...
#[derive(Clone, Debug, PartialEq)]
pub(crate) struct WorldgenShowcaseOptions {
...
impl HeadlessScreenshotUi {
    pub(crate) fn game_screen(self) -> Option<GameScreen> {
        match self {
            ...
            Self::Death => Some(GameScreen::Death {
            ...
            Self::OptionsLocalPlayPause => Some(GameScreen::OptionsCategory {
                parent: GameOptionsParent::Pause,
            ...
            Self::StorageFactoryConfirm => Some(GameScreen::StorageConfirm {
                parent: GameOptionsParent::Title,
...
impl StartupWaitPolicy {
    ...
    pub(crate) const OFFSCREEN_SCREENSHOT_DEFAULT: Self = Self::ViewSettled;

    pub(crate) fn offscreen_capture_frame_count(self) -> usize {
        match self {
            Self::Frames(frames) => (frames as usize).saturating_add(1),
            Self::None | Self::Progress | Self::Playable | Self::ViewSettled => 1,
        }
    }
}
...
impl Cli {
    pub(crate) fn parse(args: impl IntoIterator<Item = String>) -> Result<Self> {
        ...
        let mut screenshot_scripted_sleep = false;
            ...
            match arg.as_str() {
                ...
                "--worldgen-showcase-card" => {
                    let path = args
                        .next()
                        .map(PathBuf::from)
                        .context("--worldgen-showcase-card requires an output directory")?;
                    if movement_perf
                        || timedemo
                        || frame_budget_probe
                        || movement_frame_probe
                    ...
                    set_headless_mode(&mut mode, HeadlessMode::WorldgenShowcase(path))?;
                }
                "--live-diorama-smoke" => {
                    let path = args
                        .next()
                ...
                "--screenshot-worldgen-lens" => {
                    screenshot_worldgen_lens =
                        parse_worldgen_lens_arg("--screenshot-worldgen-lens", args.next())?;
                }
                "--screenshot-player-box" => {
                ...
                "--screenshot-scripted-sleep" => {
                    screenshot_scripted_sleep =
                        parse_bool_arg("--screenshot-scripted-sleep", args.next())?;
                }
                ...
                "--window-camera-velocity" => {
                    window_camera_velocity = Some(parse_f32_vec3_arg(&arg, args.next())?);
                }
                "--platform-profile" => {
        ...
        if window_frame_report_seconds.is_some() && window_frame_report_path.is_none() {
            bail!("--window-frame-report-seconds requires --window-frame-report");
        }
        if window_frame_report_frames_explicit && window_frame_report_seconds.is_some() {
            bail!(
        ...
        if let Some(velocity) = window_camera_velocity {
            if window_camera_pose.is_none() {
                bail!(
                    "--window-camera-velocity requires --window-camera-eye and --window-camera-target"
                );
            }
            if velocity == [0.0; 3] {
                bail!("--window-camera-velocity must contain a non-zero component");
            }
        }
        ...
        if screenshot_scripted_interaction && screenshot_scripted_sleep {
            bail!(
                "--screenshot-scripted-interaction and --screenshot-scripted-sleep are mutually exclusive"
            );
        }
        ...
        match mode {
            ...
            Some(HeadlessMode::WorldgenShowcase(directory)) => {
                if scene.remote_addr.is_some() || scene.world_dir.is_some() {
                    bail!("--worldgen-showcase-card requires a transient local integrated world");
                }
                if scene.live_diorama.is_some() || scene.warm_world_standby_seed.is_some() {
                    bail!(
                        "--worldgen-showcase-card cannot be combined with retained-world presentations"
                    );
                }
                if scene.day_time_override.is_none() {
                    scene.day_time_override = Some(6000);
                }
                scene.render_distance = scene
                    .render_distance
                    .max(MIN_WORLDGEN_SHOWCASE_RENDER_DISTANCE);
                scene.freeze_time = true;
                scene.debug_passive_showcase = false;
                scene.adaptive_render_admission_budget = false;
                Ok(Self::WorldgenShowcase {
                    options: WorldgenShowcaseOptions {
                        directory,
                        width: width.unwrap_or(640),
                        height: height.unwrap_or(400),
            ...
            Some(HeadlessMode::Screenshot(path)) => Ok(Self::HeadlessScreenshot {
                options: HeadlessScreenshotOptions {
                    ...
                    ui: screenshot_ui,
                    hud: screenshot_hud
                        || screenshot_frame_pipeline_overlay
                        || screenshot_debug_pane
                        || screenshot_scripted_sleep,
                    frame_pipeline_overlay: screenshot_frame_pipeline_overlay,
                    debug_pane: screenshot_debug_pane,
                    worldgen_lens: screenshot_worldgen_lens,
                    ...
                    scripted_sleep: screenshot_scripted_sleep,
...
fn parse_window_frame_report_seconds_arg(flag: &str, value: Option<String>) -> Result<f64> {
    let raw = value.with_context(|| format!("{flag} requires seconds"))?;
    let seconds = raw
        .parse::<f64>()
        .with_context(|| format!("invalid {flag} value `{raw}`"))?;
    if !seconds.is_finite() || seconds <= 0.0 || seconds > MAX_WINDOW_FRAME_REPORT_SECONDS {
...
fn parse_startup_wait_arg(flag: &str, value: Option<String>) -> Result<StartupWaitPolicy> {
    ...
    match value.as_str() {
        ...
        "view-settled" => Ok(StartupWaitPolicy::ViewSettled),
        _ => {
            let frames = value
                .strip_prefix("frames:")
                ...
                .with_context(|| {
                    format!(
                        "{flag} must be none, progress, playable, view-settled, or frames:N, got `{value}`"
                    )
                })?;
            let frames = frames
                .parse::<u32>()
...
pub(crate) fn parse_screenshot_ui_arg(
    ...
    match value.as_str() {
        ...
        "death" => Ok(HeadlessScreenshotUi::Death),
        ...
        "options-local-play" | "options_local_play" => {
        ...
        "storage-factory-confirm" | "storage_factory_confirm" => {
            Ok(HeadlessScreenshotUi::StorageFactoryConfirm)
        }
...
pub(crate) fn parse_worldgen_lens_arg(
    flag: &str,
    value: Option<String>,
) -> Result<Option<WorldgenLensLayer>> {
    match value
        .with_context(|| format!("{flag} requires biome, landform, surface, hydrology, or off"))?
        .trim()
    {
        "off" | "none" | "false" | "0" => Ok(None),
        "biome" => Ok(Some(WorldgenLensLayer::Biome)),
        "landform" | "form" => Ok(Some(WorldgenLensLayer::Landform)),
        "surface" => Ok(Some(WorldgenLensLayer::Surface)),
        "hydrology" | "water" => Ok(Some(WorldgenLensLayer::Hydrology)),
        value => {
            bail!("{flag} requires biome, landform, surface, hydrology, or off, got `{value}`")
        }
    }
}
        ...
        "mclone-native-client\n\n\
         Usage:\n\
          ...
          mclone-native-client --screenshot /tmp/mclone-frame.png [--asset-pack saved|original] [--width 1280] [--height 720] [--startup-wait none|progress|playable|view-settled|frames:N] [--warm-world-standby-seed -98765] [--screenshot-ui none|title|world-list|world-create|world-delete-confirm|new-world|join-remote|pause|death|help|controls|block-palette|options-title|options-pause|options-local-play|options-seasonal-debug|options-celestial-debug|storage-profile-title|storage-factory-confirm|server-settings-pause|asset-packs-pause] [--screenshot-hud true|false] [--screenshot-frame-pipeline-overlay true|false] [--screenshot-debug-pane true|false] [--screenshot-worldgen-lens off|biome|landform|surface|hydrology] [--screenshot-player-box true|false] [--screenshot-blink-debug true|false] [--screenshot-controller-focus true|false] [--screenshot-scripted-interaction true|false] [--screenshot-scripted-sleep true|false] [--screenshot-settle-ms 0] [--screenshot-settle-frames 0] [--screenshot-terrain-horizon-diagnostic natural|ownership-level|topology|albedo|environmental-illumination|geometric-shade|local-occlusion|water|texture|frontier-support|frontier-fallback] [--screenshot-eye x,y,z] [--screenshot-target x,y,z] [--screenshot-camera-view first-person|third-person] [--season-preview true|false] [--season-appearance true|false] [--season-orbital-phase 0..1] [--season-latitude-source world|manual] [--season-latitude -90..90] [--season-solar-time-source world-clock|manual] [--season-solar-time 0..<24] [--season-recent-snow 0..1] [--season-recent-snow-center x,z] [--celestial-sun true|false] [--celestial-sun-halo true|false] [--celestial-horizon-glow true|false] [--celestial-moon true|false] [--celestial-moonlight true|false] [--celestial-stars off|quarter|half|full] [--moon-phase-source world-clock|manual-preview] [--moon-phase 0..1] [--first-person-player true|false] [--seed 12345] [--generation-profile mclone-overworld-v1|mclone-overworld-v2|mclone-overworld-v3] [--starter-content wild|intro-homestead-v1] [--chunk-x 0] [--chunk-z 0] [--render-distance 5] [--movement-speed-multiplier 1.0] [--simulation-cadence 20/20/60] [--debug-passive-showcase true|false] [--section-occlusion true|false] [--lighting true|false] [--fullbright true|false]\n\
           ...
           mclone-native-client --desktop-xr [--no-window] [--frames N|--xr-forever] [--view-pose X,Y,Z,YAW_DEGREES] [--xr-underwater-mode midpoint|per-eye] [--xr-debug-ui none|pause|controls|graphics|seasonal-debug] [--xr-render-mode dual-per-eye|array-per-eye|array-multiview] [--xr-render-mode-cycle] [scene/render options as --xr-mclone-smoke]\n\n\
