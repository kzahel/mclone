    ...
    pub fn expose_adapter(
        ...
        let private_caps = super::PrivateCapabilities {
            flip_y_requires_shift: phd_capabilities.device_api_version >= vk::API_VERSION_1_1
                || phd_capabilities.supports_extension(khr::maintenance1::NAME),
            // Mclone XR workaround: wgpu-hal 25's imageless framebuffer path
            // produces zeroed Quest/Adreno multiview attachments. Use ordinary
            // framebuffers until the workspace moves to a wgpu version whose
            // Vulkan framebuffer path is known to pass the Quest multiview
            // readback proof.
