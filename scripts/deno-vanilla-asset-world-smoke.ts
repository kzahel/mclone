...
await renderWorldWorker.initialize({
  ...
  height: DENO_VANILLA_ASSET_WORLD_HEIGHT,
});
...
const biomeSource = new OverworldBiomeSource(DENO_VANILLA_ASSET_WORLD_SEED);
const mainThreadLevel = new ClientChunkCache({
  airState: blocks.airState,
  minBuildHeight: DENO_VANILLA_ASSET_WORLD_MIN_BUILD_HEIGHT,
  height: DENO_VANILLA_ASSET_WORLD_HEIGHT,
  biomeSource,
  biomeZoomSeed: DENO_VANILLA_ASSET_WORLD_SEED,
...
const viewArea = new ViewArea(chunkDispatcher, mainThreadLevel, DENO_VANILLA_ASSET_WORLD_VIEW_DISTANCE, levelRenderer);
levelRenderer.setLevel(mainThreadLevel, chunkDispatcher, viewArea, DENO_VANILLA_ASSET_WORLD_VIEW_DISTANCE);
...
if (nonClearPixels < MIN_NON_CLEAR_PIXELS) {
  throw new Error(`Deno vanilla asset world smoke rendered too few non-clear pixels: ${nonClearPixels.toString()}`);
}
assertStoneTextureLike(centerPixel);
...
function applyDirtySections(
  ...
  for (const dirtySection of sink.drainDirtySections()) {
    if (
      dirtySection.y < DENO_VANILLA_ASSET_WORLD_MIN_BUILD_HEIGHT ||
      dirtySection.y >= DENO_VANILLA_ASSET_WORLD_MIN_BUILD_HEIGHT + DENO_VANILLA_ASSET_WORLD_HEIGHT
    ) {
      continue;
    }
...
function createHeadlessScene(
  ...
  return {
    ...
    worldBounds: {
      minBuildHeight: DENO_VANILLA_ASSET_WORLD_MIN_BUILD_HEIGHT,
      maxBuildHeight: DENO_VANILLA_ASSET_WORLD_MIN_BUILD_HEIGHT + DENO_VANILLA_ASSET_WORLD_HEIGHT,
    },
