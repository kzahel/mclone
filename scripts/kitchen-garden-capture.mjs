...
const root = "/tmp/mclone-kitchen-garden-fixture";
const image = "/tmp/mclone-kitchen-garden.png";

const fixtureOutput = run([
  "cargo", "run", "--manifest-path", "native/Cargo.toml", "-p", "mclone-server",
  "--bin", "playable_showcase_fixture", "--", "--showcase", "kitchen-garden",
  "--root", root,
]);
...
if (receipt.id !== "kitchen-garden" || receipt.revision !== "1") {
  throw new Error("showcase compiler did not emit the expected kitchen-garden receipt");
}
...
if (!/GUI commands/.test(output)) {
  throw new Error("kitchen-garden capture did not include the shared HUD");
}
if (!existsSync(image) || statSync(image).size < 10_000) {
  throw new Error(`kitchen-garden capture did not write credible pixels to ${image}`);
}
console.log(
  `MCLONE_KITCHEN_GARDEN_CAPTURE id=${receipt.id} revision=${receipt.revision} seed=${receipt.seed} entry_eye=${receipt.entry_eye} entry_target=${receipt.entry_target} flat=${image}`,
);
