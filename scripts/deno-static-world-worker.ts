...
import {
  ...
  createDenoStaticWorldWorkerStoneSprite,
} from "./deno-static-world-smoke-shared.ts";

const SECTION_MESH_PADDING_BLOCKS = 2;
const SECTION_MESH_MAX_BLOCK_OFFSET = 17;

interface DenoStaticRenderWorldContext {
  readonly level: ClientChunkCache;
  readonly blockRenderer: ReturnType<typeof createDenoStaticWorldBlockRenderer>;
}

let context: DenoStaticRenderWorldContext | undefined;
...
function getContext(messageType: string): DenoStaticRenderWorldContext {
  if (context === undefined) {
...
function createContext(request: InitializeRenderWorldRequest): DenoStaticRenderWorldContext {
  ...
  const stoneSprite = createDenoStaticWorldWorkerStoneSprite();
  return {
    level,
    blockRenderer: createDenoStaticWorldBlockRenderer(
      stoneSprite,
...
function ingestUpdates(
  context: DenoStaticRenderWorldContext,
