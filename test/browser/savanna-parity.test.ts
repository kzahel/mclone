...
const SAVANNA_SCREENSHOT_PATH = "/tmp/mclone-debug-savanna.png";
...
test("debug free-cam captures savanna vegetation in the worker-generated world", async ({ page }) => {
  await page.goto(
  ...
  await page.locator("#renderer").screenshot({ path: SAVANNA_SCREENSHOT_PATH });

  const state = await readDebugState(page);
  expect(state.error).toBeUndefined();
  expect(state.worldTransport).toBe("worker");
  expect(state.loadedChunkCount).toBeGreaterThan(0);
  expect(state.playerChunkX).toBe(17);
  expect(state.playerChunkZ).toBe(-49);
});
