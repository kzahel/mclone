...
function isAttachable(state: BlockState, level: BlockGetter, pos: BlockPos): boolean {
  ...
  return location === KELP_PLANT_LOCATION.toString() || (state.isFaceSturdy(level, pos, Direction.UP) && location !== MAGMA_BLOCK_LOCATION.toString());
}

export class KelpBlock extends Block {
  public static readonly AGE = IntegerProperty.create("age", 0, 25);

  public constructor(properties: BlockBehaviour.Properties) {
    super(properties);
    this.registerDefaultState(this.stateDefinition.any().setValue(KelpBlock.AGE, 0));
  }
